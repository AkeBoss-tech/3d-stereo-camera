...
plt.imsave('output/stereo_disparity_map.png', disparity_color)
plt.show()
...
plt.title('Depth Map')
plt.imsave('output/stereo_depth_map.png', depth_color)
plt.show()

# Normalize the depth map for display (in centimeters)
depth_normalized_cm = cv2.normalize(depth_normalized, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)

# Display the depth map with grayscale color scale and units (in centimeters)
plt.imshow(depth_normalized_cm, cmap='gray')
cbar = plt.colorbar(label='Depth (cm)')
plt.title('Grayscale Depth Map (cm)')
plt.show()

# Save grayscale depth map to file
cv2.imwrite('depth_map_gray.png', depth_normalized_cm)
...
plt.savefig('output/stereo_depth_map_annotated.png')
plt.show()